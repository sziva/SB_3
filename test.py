...
def load_model(path):
    json_file = open(path + 'model500.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()

    model = model_from_json(loaded_model_json)
    model.load_weights(path + "model500.h5")
...
fcc_path = "haarcascade_frontalface_alt.xml"
...
while True:
    ...
    for (count, (x, y, w, h)) in enumerate(faces):
        ...
        cv2.putText(frame, "Face #" + str(count + 1), (x + 5, y + h + 11), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
        ...
        cv2.putText(frame, emotion + ": " + str(confidence) + "%", (x + 20, y + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (255, 255, 255), lineType=cv2.LINE_AA)
